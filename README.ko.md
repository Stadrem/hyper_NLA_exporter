...
* **Marker Segments**: 타임라인 마커를 기반으로 감지된 애니메이션 클립들을 표시합니다. 클립 이름 버튼을 클릭하면 타임라인의 재생 구간이 해당 클립의 시작/끝 프레임으로 자동 설정되어 빠르게 미리볼 수 있습니다.
  * *Reset Range*: 타임라인 재생 범위를 0프레임부터 마지막 마커 프레임까지 전체 애니메이션 길이로 복원합니다.
...
* **Settings (설정)**:
  ...
  * *Selected Only*: 체크 시 선택된 오브젝트만 추출 대상으로 삼습니다.
* **Quick Export (Marker Split)**:
...
1. **GLB/glTF 계층 구조 및 스케일 유지**:
   * 이 애드온의 GLB 퀵 익스포트(`Quick Export GLB`)는 내부적으로 `use_selection=False` (씬 전체 내보내기)를 강제 적용합니다.
   * 블렌더의 glTF 익스포터는 NLA 트랙 모드에서 스킨드 메시(Skinned Mesh)의 본(Bone) 부모 매핑 계층 정보를 완벽하게 빌드하기 위해 씬 구조 전체를 참조해야 합니다. 선택 오브젝트만 강제 내보내기를 할 경우 메시가 복제되거나 결합되지 않고 잘못된 위치에 렌더링되는 엔진 제약이 있습니다.
...
2. **Blender 5.1 Layered Action 애니메이션 구조**:
   * 블렌더 5.x의 최신 데이터 레이아웃(Action $\rightarrow$ Slot $\rightarrow$ Layer $\rightarrow$ Strip $\rightarrow$ Channelbag)을 지원하여, 다중 슬롯 캐릭터 간의 이름 충돌을 원천 차단합니다.
